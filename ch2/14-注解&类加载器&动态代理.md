...
>
> ![](http://p35l3ejfq.bkt.clouddn.com/18-6-3/44453277.jpg)
>
> 可以看到注解实质就是一个接口。
...
>
> ![](http://p35l3ejfq.bkt.clouddn.com/18-6-3/1700750.jpg)
>
> 可以看到该 web 项目的 WEB-INF 目录下是没有 web.xml 文件的。然后可以新建 servlet3.0 了。
>
> 如图，也可勾选
>
> ![](http://p35l3ejfq.bkt.clouddn.com/18-8-19/24373101.jpg)
>
> WEB-INF 目录下会存在 web.xml 文件。（关于2.x 与 3.x 区别参考网上资料，如 [dynamic web module version 2.5与3.0的主要区别](https://blog.csdn.net/qq_39056805/article/details/79694331)）
...
其中 `UploadUtils.java`  ：（这个封装的工具类可以留着以后用到）
...
> 我们编写的`.java`文件，jvm 会将它变成`.class`文件。该文件要想运行，必须加载内存中，然后会生成一个对象 .Class 对象。
...
![](http://p35l3ejfq.bkt.clouddn.com/18-6-3/30292295.jpg)

如下可以看到 `rt.jar` 包下很多类的字节码文件。

![](http://p35l3ejfq.bkt.clouddn.com/18-6-3/55790824.jpg)
...
![](http://p35l3ejfq.bkt.clouddn.com/18-6-3/2814485.jpg)



能观察到依次由`应用类加载器`、`扩展类加载器`加载。	PS：看到打印的为 null ，表示使用的`引导加载器`。
...
为什么`final QQ qq=new QQ();`  QQ对象声明为 final 呢？

> 这是因为创建完代理对象后还需要做其操作，需要保证代理对象不能被删除，即 QQ 对象 qq 存在。
